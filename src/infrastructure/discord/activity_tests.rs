...
fn first_snapshot_summarizes_the_channel() {
    ...
    let lines = diff_lines(None, &next);
...
fn channel_change_is_a_single_summary() {
    ...
    let lines = diff_lines(Some(&prev), &next);
    assert_eq!(lines.len(), 1);
...
fn leaving_voice_logs_cleared() {
    ...
    assert_eq!(diff_lines(Some(&prev), &next), vec!["← left voice (roster cleared)"]);
}
...
#[test]
fn speaking_transitions_are_logged_both_ways() {
    let quiet = roster(Some("1"), vec![member("a", "nina", false)]);
    let talking = roster(Some("1"), vec![member("a", "nina", true)]);
    assert!(diff_lines(Some(&quiet), &talking).iter().any(|l| l.contains("🎤 nina")));
    assert!(diff_lines(Some(&talking), &quiet).iter().any(|l| l.contains("nina stopped")));
}

#[test]
fn no_change_yields_no_lines() {
    let r = roster(Some("1"), vec![member("a", "nina", false)]);
    assert!(diff_lines(Some(&r), &r).is_empty());
}
