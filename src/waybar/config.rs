...
pub fn stream_bar_object(output: &str) -> Value {
    ...
    let exec_script = format!(
        concat!(
            ...
            "python3 -c \"",
            "import json, sys;",
            "LABELS = {{'follow':'follow','newSubscriber':'sub','recurringSubscriber':'resub',",
            "'giftSubscriber':'gift','directGiftSubscriber':'gift',",
            "'donation':'tip','cheer':'cheer','raid':'raid'}};",
            "f = open('{data}');events = json.load(f);f.close();",
            "parts = [];",
            "[parts.append(",
            "e.get('username','?') + ' ' + LABELS.get(e.get('event_type',''),e.get('event_type',''))",
            " + (' ' + e['amount'] if e.get('amount') else '')",
            ") for e in events];",
            "text = '  //  '.join(parts) if parts else '';",
            "print(json.dumps({{'text': text, 'tooltip': str(len(events))+' recent events'}}))",
            "\"; ",
            "else echo '{{\"text\":\"\",\"tooltip\":\"No events\"}}'; fi",
        ),
        data = data_path,
    );
