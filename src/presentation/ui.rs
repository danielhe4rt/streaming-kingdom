...
pub fn draw(frame: &mut Frame, app: &AppState, tui: &TuiState) {
    ...
    let [top_right, event_log_area] = Layout::vertical([
    ...
    let [stats_area, chat_area] = Layout::horizontal([
...
fn draw_integrations(frame: &mut Frame, area: Rect, app: &AppState, tui: &TuiState) {
    ...
    {
        let cursor = if focused && tui.integration_cursor == 0 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            ...
            Span::styled("Twitch EventSub", Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
            Span::raw(if tui.twitch_eventsub.connected { " Connected" } else { " Disconnected" }),
        ]));
        if tui.twitch_eventsub.subs_registered > 0 || tui.twitch_eventsub.subs_total > 0 {
            let subs_text = if tui.twitch_eventsub.subs_total > 0 {
                format!("    Subs: {}/{}", tui.twitch_eventsub.subs_registered, tui.twitch_eventsub.subs_total)
            } else {
                format!("    Subs: {}", tui.twitch_eventsub.subs_registered)
            };
            lines.push(Line::from(Span::styled(subs_text, Style::default().fg(Color::DarkGray))));
        }
    ...
    {
        let cursor = if focused && tui.integration_cursor == 1 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            Span::raw(format!("{cursor} ")),
            Span::styled("Twitch Chat (IRC)", Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
            Span::raw(if tui.twitch_chat.connected { " Connected" } else { " Disconnected" }),
        ]));
    ...
    {
        let cursor = if focused && tui.integration_cursor == 2 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            Span::raw(format!("{cursor} ")),
            Span::styled(format!("{checkbox} Livepix Donations"), Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
    ...
    {
        let cursor = if focused && tui.integration_cursor == 3 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            Span::raw(format!("{cursor} ")),
            Span::styled(format!("{checkbox} Privacy Monitor"), Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
            Span::raw(if tui.privacy.running { " Running" } else { " Stopped" }),
        ]));
        if tui.privacy.running {
            ...
            let blur_color = if tui.privacy.blur_active { COLOR_DISCONNECTED } else { COLOR_CONNECTED };
            lines.push(Line::from(Span::styled(blur_text, Style::default().fg(blur_color))));
        }
        lines.push(Line::from(""));
    }
    ...
    {
        let cursor = if focused && tui.integration_cursor == 4 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            Span::raw(format!("{cursor} ")),
            Span::styled(format!("{checkbox} Waybar"), Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
            Span::raw(if app.waybar_enabled { " Active" } else { " Inactive" }),
        ]));
        lines.push(Line::from(""));
    }
    ...
    {
        let cursor = if focused && tui.integration_cursor == 5 { ">" } else { " " };
        ...
        lines.push(Line::from(vec![
            Span::raw(format!("{cursor} ")),
            Span::styled(format!("{checkbox} Hyprland"), Style::default().add_modifier(Modifier::BOLD)),
        ]));
        lines.push(Line::from(vec![
            Span::raw("    "),
            Span::styled(dot, Style::default().fg(dot_color)),
            Span::raw(if tui.hyprland.listening { " Listening" } else { " Inactive" }),
        ]));
...
fn draw_stats(frame: &mut Frame, area: Rect, app: &AppState, tui: &TuiState) {
    ...
    let [counters_area, highlights_area] = Layout::vertical([
        Constraint::Length(9),
        Constraint::Min(0),
    ])
    .areas(inner);
    ...
    frame.render_widget(Paragraph::new(highlight_lines).wrap(Wrap { trim: true }), highlights_inner);
}
...
fn format_app_event(entry: &AppEventEntry) -> Line<'static> {
    ...
    let (icon, color, body) = match &entry.event {
        ...
        // --- Livepix group ---
        AppEvent::LivepixInfo(msg) => (
            "ℹ",
            COLOR_INFO,
            format!("Livepix: {msg}"),
        ),
        AppEvent::LivepixError(msg) => (
            "✖",
            COLOR_ERROR,
            format!("Livepix: {msg}"),
        ),
